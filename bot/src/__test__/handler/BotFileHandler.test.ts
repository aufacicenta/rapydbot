...
describe("BotFileHandler", () => {
  ...
  test("Decrypt image file", async () => {
    ...
    const passportData = handler.decipherData<{
      ...
      {
        secret: Buffer.from(credentials.secure_data.passport.data.secret, "base64"),
        hash: credentials.secure_data.passport.data.data_hash,
      },
      encryptedPassportData.data.filter((d) => d.type === "passport")[0].data as string
    );

    const passportFile = handler.decipherFile(
      {
        secret: Buffer.from(credentials.secure_data.passport.front_side.secret, "base64"),
