import { translationKeys } from "../../translationKeys";

export default {
  [translationKeys.command_text_set_country]: `Selecciona un <strong>país</strong> de la lista.

Esto mostrará las opciones de pago correspondientes al país en la página de pago.


<em>Your transactions are secured by <a href="https://www.rapyd.net/platform/protect/">Rapyd Protect</a>.</em>
  `,
  [translationKeys.command_text_set_currency]: `Selecciona una <strong>moneda</strong> de la lista.

Esto mostrará las opciones de pago correspondientes al país en la página de pago.


<em>Your transactions are secured by <a href="https://www.rapyd.net/platform/protect/">Rapyd Protect</a>.</em>
  `,
};
